...
const columns = [
  { title: 'Tên sản phẩm', dataIndex: 'name', key: 'name', width: '46%', ellipsis: true },
  { title: 'Doanh số', dataIndex: 'total', key: 'total', colSpan: '16%', align:'center', },
  { title: 'Giá(Đơn vị: vnd)', dataIndex: 'price', key: 'price',colSpan: '16%', align:'center', },
  { title: 'Kho hàng', dataIndex: 'quantity', key: 'quantity',colSpan: '16%', align:'center', },
  { title: 'Thao tác', dataIndex: 'tt', key: 'tt' ,colSpan: '16%', align:'center',},
];
...
const onExpand = (expanded, record) => {
    ...
    if (expanded) {
        ...
        const findInner = products.value.find(product => product.product_id === record.key);
...
const getProductSeller = async () => {
    try {
        const res = await mainServices.getProductSoldOutSeller();
        //console.log(res);
        products.value = res.data.result.content;
        total.value = res.data.result.totalElements;
        pageSize.value = res.data.result.size;
        updateData();
    } catch (err) {
        console.log(err);
    }
      ...
      <template v-if="column.key === 'name'">
        <div class="flex items-center">
            <img
                ...
                :src="record.imgUrl?`http://${record.imgUrl}`:defaultProduct"
                alt=""
            />
            <p
        ...
        <template v-if="column.key === 'total'">
            <span class="text-[1.8rem]">{{record.sold? record.sold*record.price:0}}</span>
        </template>
        ...
        <template v-if="column.key === 'tt'" >
            <div class="flex flex-col items-center gap-3">
                <a-button @click="deleteProduct(record.key)" type="primary" danger>Xoá</a-button>
            </div>
        </template>
    </template>
    <template #expandedRowRender="{record}">
      <a-table :columns="innerColumns[record.key]" :data-source="innerData[record.key]" :pagination="false">
        <template #bodyCell="{ column, record }">
