...
const onSubmit = async() => {
    ...
    try {
        ...
        const res = await authServices.otp({email: email.value,otp: otp.value});
        console.log(res);
...
<template>
    ...
    >
        <div 
        class="p-10 bg-white rounded flex flex-col w-[400px]"
        >
            <p class="text-[1.6rem] leading-relaxed">Vui lòng nhập otp để kích hoạt tài khoản. Otp có hiệu lực 2 phút.</p>
            <input 
            v-model="otp"
            type="text"
            placeholder="Otp"
